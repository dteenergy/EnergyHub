sap.ui.define([
	...
	"sap/m/Dialog",
	"sap/m/MessageToast"
], (BaseController, Fragment, JSONModel, GlobalInputValues, ChecksInputValidation, FormatInputs, Dialog, MessageToast) => {
	"use strict";
	...
	return BaseController.extend("dteconsentappclient.controller.Enrollment", {
		...
		// Callback to trigger the Recaptcha verification.
		_onRecaptchaSuccess: async function (token) {
		...
		additionalLocationAlert: function () {
			...
			if (!oErrorVisibilityModelData?.isInputInValid && !oErrorVisibilityModelData?.isTermsAndConditionVerifiedStatus || recaptchaStatus !== false) {
				...
				const dialogContent = new sap.m.FlexBox({
					items: [
						...
						new sap.ui.core.HTML({
							id: "recaptchaPlaceholder",
							content: "<div id='recaptchaDiv'></div>", // Placeholder div for reCAPTCHA
